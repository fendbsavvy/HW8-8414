...
**Usage Instructions**
    (train_model.py)
    - Train classification model → saved as `phishing_url_detector`.  
    - Train clustering model → saved as `threat_actor_profiler`.
    - Access web application http://<IP `or localhost`>:8501.  
    - Select the characteristics of the suspicious URL from the side menu.
    - Select Gemini as GenAI Provide
    - Click "Analyze & Initiate Response" button
    - Allow the model to complete analysis, view results in the analysis tabs.
